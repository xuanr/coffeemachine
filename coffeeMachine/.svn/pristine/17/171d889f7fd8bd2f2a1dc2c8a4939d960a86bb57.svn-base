        ...
        if (IOS7) {
            CGRect stringRect = [string boundingRectWithSize:constraintSize
                                                  ...
                                                  attributes:@{NSFontAttributeName: self.stringLabel.font}
                                                     context:NULL];
            size = stringRect.size;
        }
        else {
            size = [string sizeWithFont:self.stringLabel.font constrainedToSize:constraintSize lineBreakMode:NSLineBreakByCharWrapping];
        }
        
        stringWidth = size.width;
        stringHeight = ceil(size.height);
